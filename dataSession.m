...
classdef dataSession < handle
	...
	properties (SetAccess = protected)
        % settings
        velocityThreshold = 10; eyeFilter = 10; RFsize = 100;
	...
	methods (Access = public)
		...
		function unpackScene(obj)
            ...
            vreader = VideoReader(obj.AVIfile);
            nFrames = 600;%vreader.NumberOfFrames;
            for ff=1:nFrames
                fprintf('\nRead video %i%%',round(100*ff/nFrames));
                pixelX  = max(round(obj.eyeX(ff)-0.5*obj.RFsize+0.5),1):min(round(obj.eyeX(ff)+0.5*obj.RFsize-0.5),obj.sceneSize(1));
                pixelY  = max(round(obj.eyeY(ff)-0.5*obj.RFsize+0.5),1):min(round(obj.eyeY(ff)+0.5*obj.RFsize-0.5),obj.sceneSize(2));
                tmp1 = NaN(obj.RFsize);
                tmp2 = rgb2gray(vreader.read(ff));
                tmp3 = tmp2(pixelY,pixelX);
              	tmp1(obj.RFsize-size(tmp3,1)+1:end,obj.RFsize-size(tmp3,2)+1:end) = tmp3;
                obj.scene(:,ff) = tmp1(:);
            end
        end
